...
{
  double sin_arg=numbers::PI;
  for(unsigned int i=0;i<dim;++i){
    sin_arg=sin_arg*p[i];

  }
  return std::cos(p[0])*std::sin(p[1])*p[2];
  // return p.square();
}
  template <int dim>
  ...
  void LaplaceProblem<dim>::output_results(const unsigned int cycle) const
  {
    ...
    data_out.write_deal_II_intermediate_in_parallel("sphere.pd2",mpi_communicator,DataOutBase::CompressionLevel::default_compression);
    data_out.write_vtu_in_parallel("sphere.vtu",mpi_communicator);
    ...
    //   {
        ...
        //   {
          Point<dim> center(0,0,0);
            GridGenerator::hyper_ball(triangulation,center,1.0);
